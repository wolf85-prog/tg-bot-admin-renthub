...
import { sendDocumentToTelegram, sendMessageToTelegram, sendPhotoToTelegram } from "src/http/telegramAPI";
	...
	const sendText = async () => {
		...
		if(!file) {
			...
			if (image.slice(-3) === 'gif' || image.slice(-3) === 'pdf' || image.slice(-3)==='zip') {
				...
				sendPhotoToTelegram = await sendPhotoToTelegram({user: person.id, image: host+image})
			}
		}
