...
const DistributionR = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setDistributionsRent(arrDitributions) 
      setLoading(false)
    }
  ...
  const removeDescription = async(desk) => {
    ...
    setDistributionsRent(distributionsRent.filter(p => p.id !== desk.id))
    
    //удаление сообщения в базе данных
    await delDistributionR(desk.id)  
        ...
        <div className="body flex-grow-1 px-3">
            ...
            <CContainer lg>
                <Suspense fallback={<CSpinner color="primary" />}>
                  <>
                    {/* <h2>Рассылки</h2> */}
                      <Link to={'/distributionr_add'}><CButton color="primary" size="lg" >Новая рассылка</CButton></Link>
